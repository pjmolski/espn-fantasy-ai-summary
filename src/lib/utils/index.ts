...
async function fetchEspnData(url: string, params: any): Promise<any> {
...
async function loadLeague(leagueId: string, year: number): Promise<LeagueData> {
...
async function iterateWeeksEspn(
	...
	for (let i = 1; i <= week; i++) {
		const weeklyDf = await loadWeeklyStats(year, leagueId, i);
...
async function runEspnWeekly(
	...
	while (attempts < maxAttempts) {
		try {
			...
			standingsDf = await iterateWeeksEspn(year, week, standingsDf, leagueId);
